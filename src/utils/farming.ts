...
export async function getFamingPoolInfo(
  ...
  if (farmingPoolInfoStorage.isV4201) {
    result = await farmingPoolInfoStorage.asV4201.get(pid)
  }
  return result
}
...
export async function getFamingSharesAndWithdrawnRewards(ctx: EventHandlerContext, pid: number, user: Uint8Array) {
  ...
  if (storage.isV4201) {
    result = await storage.asV4201.get(BigInt(pid), user)
  }
  return result
}
...
export async function updateFarmingPoolInfo(ctx: EventHandlerContext, pid: bigint) {
  ...
  const lpToken = await lpToAssetIdPairStorage.asV4201.get(farmingToken)
